...
The header does not read rows from the block element directly. Instead, it loads a fragment from `/_fragments/nav/header` (or a path specified in the `header` page metadata). The fragment is expected to contain three sections:
...
The language selector lazy-loads a sub-fragment from `/_fragments/nav/header/languages` when clicked.
...
Authors do not create the header in page content. It is loaded automatically as a global fragment. The fragment document at `/_fragments/nav/header` contains the brand, nav list, and action links.
...
Like the header, the footer loads its content from a fragment at `/_fragments/nav/footer` (or a path specified in the `footer` page metadata). The fragment is expected to contain multiple sections:
...
Authors do not create the footer in page content. It is loaded automatically as a global fragment. The fragment document at `/_fragments/nav/footer` contains footer content sections, legal links, and copyright text.
