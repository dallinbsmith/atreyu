...
| 4 | `loadExperience` | Rewritten as a promise chain calling `mod.default(el)` | Style | Same behavior: errors still go to `log(ex, el)`. This is the call site B1 changes. | No |
...
| 7 | `decoratePictures` | `data-grid-source` re-run guard | Upstream | On a re-run, the prepended `<source>` was cloned again, adding one duplicate per run. Upstream addressed a similar problem another way in `16f193e` (`picture:has([loading])`); reconcile the two before opening a PR. | Yes (after reconciling) |
...
| 11 | `decorateButton` | Underline button: `link.innerHTML = isUnder.innerHTML` becomes `link.append(...isUnder.childNodes)` | Upstream | Removes an `innerHTML` sink and keeps the original nodes. | Yes |
...
| 17 | `slugifyUnique` (new export) | `slugifyUnique(text, root = document)` | Convention | Section anchors and `header-subcategories.js`. `root` lets detached fragment subtrees de-dupe ids against each other. | No |
| 18 | `decorateSection` | Section-metadata `anchor` key becomes `section.id` | Convention | Deep links and jump nav. C4 moves this out of `ak.js`. | No |
| 19 | `decorateSection` | Dataset key normalization (`toClassName`, then camelCase) | Upstream | A multi-word key such as "Experiment Variants" threw `InvalidCharacterError` and stopped decoration of every later section. | No (upstream deleted this code in `345f6ea`) |
...
| 22 | `decorateSkipToContent` (new), `decorateDoc` | Skip link, with a re-run guard | Convention | Accessibility. The whole function is ours, not just the guard: upstream has no skip link. It uses a hard-coded English label. | No |
...
| 24 | throughout | Arrow functions, `?.`, `.at(0)`, `loadStyle` via `Promise.withResolvers()`, `map`/`filter` in `decorateLinks`, early returns in `decorateSection`, upstream comments deleted | Style | House lint style (`prefer-arrow-functions`). `loadStyle` still resolves immediately when the link already exists, as the base did (`null` now, `undefined` before; no caller reads the value). | No |
...
| --- | --- | --- | --- |
| B1 | `loadExperience`, `loadArea` | Block teardown signal: an iterable `Map<Element, AbortController>`. `loadExperience` passes `mod.default(el, { signal })`. `teardownDetached()` sweeps disconnected entries (skipping detached `.fragment-content` roots) at the start of each document-level `loadArea`. | Convention (could go upstream later) |
| B2 | `decorateSection` | **Pending re-scope:** align the client parser with the server's `toMetaName` rules for the section-metadata tables our own code builds in the browser. The original B2 (stop lowercasing values) is part of that, since the server keeps values as authored. | Convention (upstream no longer has a client parser) |
...
2. **Anyone editing `ak.js` must update this file in the same PR:** add or change the row and set
   the classification. CI fails a pull request that changes `scripts/ak.js` without changing this
   file.
