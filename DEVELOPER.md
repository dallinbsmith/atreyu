...
| `i18n.js` | `formatDate(value, opts)` | The path for user-visible dates. Formats with `Intl.DateTimeFormat` in the page locale (`getConfig().locale.lang`, falling back to `'en'`, never the browser locale). Default is a long date ("September 24, 2026"); `opts` are merged over it and passed to `Intl.DateTimeFormat` (e.g. `{ month: 'short' }`, `{ timeZone }`, or `{ dateStyle }`). `YYYY-MM-DD` input stays on its calendar day in every time zone (a caller `timeZone` is ignored for `YYYY-MM-DD`). Invalid or empty input is returned unchanged. |
