...
- One directory per block. Its default export is its only public surface: `(el) => {}` or `async (el) => {}`. A block that holds listeners, observers or timers outside its own subtree, or appends nodes to `body`, takes `(el, { signal } = {})` and cleans up when `signal` aborts. Always keep the `= {}` default
- Split by concern, not line count: once a file mixes concerns, move helpers to sibling files in the block's folder. Reuse `scripts/utils/` first; a *generic* helper moves to `scripts/utils/` once it has a second caller; a block-specific helper another block needs usually means a variant. ESLint's `max-lines` (200, blank lines and comments not counted) is a backstop, not the target
