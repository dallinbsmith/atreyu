...
- Split by concern, not line count: once a file mixes concerns, move helpers to sibling files in the block's folder. Reuse `scripts/utils/` first; a helper moves there only once it has a second caller. ESLint's `max-lines` (200, blank lines and comments not counted) is a backstop, not the target
