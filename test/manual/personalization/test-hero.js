// Manual personalization test harness — page bootstrap, not production code.
//
// Reproduces the one piece of real ak.js behavior this harness exists to
// interact with correctly: a section stays hidden (styles/styles.css:
// `div[data-status] { display: none }`, which out-specifies `.section {
// display: block }` only while `data-status` is present) until every block
// inside it finishes decorating (ak.js's decorateSections()/loadArea()
// `await Promise.all(section.blocks.map(loadBlock))` before `delete
// section.dataset.status`). decoratePznSlots() must run AFTER that reveal,
// and must not be folded into the awaited block-loading step — see the
// comment on decoratePznSlots in scripts/utils/analytics/pzn.js for why.
//
// Everything upstream of "section already has a .block-content > .hero shape"
// (raw table parsing, groupChildren(), etc.) is skipped — this harness starts
// one step downstream of document authoring, see test-hero.html.

import decorateHero from '../../../blocks/hero/hero.js';
// pzn.js graduated out of a spike into scripts/utils/ on 2026-08-28 — this
...
// real LCP/CLS measurement pass isn't scoring this harness's own QA chrome
// (the panel's flex-wrapping buttons/dl elements reflow as their text
// content updates, which shows up as unrelated layout-shift entries if left
// in the DOM — found by measuring, not assumed; see the original spike
// report).
